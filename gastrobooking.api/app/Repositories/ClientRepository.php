    ...
    public function sendRequest($request) {
        ...
        if ($request->mealslist) {
            foreach ($request->mealslist as $value) {
                ...
                $x_number = ((isset($value['x_number']))) ? $value['x_number'] : '';
        ...
        $requests->ID_restaurant = (($request->new_restaurant_id)) ? $request->new_restaurant_id : '';
        ...
        $sent_rest = $this->sendEmailReminder('new', app('Dingo\Api\Auth\Auth')->user(), $order, $restaurant,
                $lang, $orders_detail_filtered, 'rest', $new_restaurant, $request_menu_filtered);

        $sent_sms = $this->sendSMSEmailReminder('new_short', app('Dingo\Api\Auth\Auth')->user(), $order, $restaurant, 'admin', $new_restaurant, $orders_detail_filtered, $request_menu_filtered, $lang);
            ...
            if ( $status == 'N' ) {
                ...
                if ($requests->save()) {
                    ...
                    if ( $countCancelRequests != 0 ) {
                        $sent_rest = $this->sendUpdateEmailReminder('update', $user, $getMealRequest, $restaurant, $lang, 'rest', $new_restaurant,$checkcancel, $checkConfirm);
                        $sent_sms = $this->sendOtherSMSEmailReminder('update_short', app('Dingo\Api\Auth\Auth')->user(), $getMealRequest, $restaurant, $lang, 'admin', $new_restaurant);
                    }
    ...
    public function cancelRequest($request){
        ...
        if ($request->ID) {
            ...
            if ($requests->save()) {
                ...
                $sent_rest = $this->sendCancelEmailReminder('cancel', app('Dingo\Api\Auth\Auth')->user(), $getMealRequest, $restaurant, $lang, 'rest', $new_restaurant);

                $sent_sms = $this->sendOtherSMSEmailReminder('cancel_short', app('Dingo\Api\Auth\Auth')->user(), $getMealRequest, $restaurant, $lang, 'admin', $new_restaurant);
