...
//    private $rest_hash =
    ...
    public function getRestaurantList(){
        return Restaurant::groupby('name')->get(['id', 'ID_user', 'name', 'lang', 'street', 'city', 'address_note']);
    }
   ...
   public function sendSMSEmailReminder($type, User $user, $getMealRequest, $restaurant, $clientLang, $to, $new_restaurant)
        ...
        foreach ($orders_detail as $order_detail) {
            if ($order_detail->status != 3){
                if ($order_detail->is_child){
                    $price += ($order_detail->price_child && $order_detail->price_child > 0 ? $order_detail->price_child : $order_detail->price) * $order_detail->x_number;
                } else {
                    $price += $order_detail->price * $order_detail->x_number;
                }
            }
    ...
    public static function getAssignments($request){
     ...
     public static function getOrders($request){
         ...
         $str = "";
         $str1 = " order by orders_detail.serve_at ASC";
         if($company !== '' && $request->company != null && $request->company == 1){
             $str = $str." AND restaurant.ID_user_dealer=$user";
         }
         if($country !== ''){
             $str = $str." AND district.country='$country'";
         }
         if($district !== ''){
           $str = $str." AND restaurant.ID_district=$district";
           
         }
         if($clientname !== ''){
            $str = $str." AND restaurant.name LIKE '%".$clientname."%' ";
         }
         if($booking == true && $orderRequest == true){
            $str = $str;
         }else{
            if($booking !== false && $booking !== '' ){
                $str = $str." AND orders_detail.ID_menu_list !='' ";
             }
             if($orderRequest !== false && $orderRequest !== '' ){
                $str = $str." AND orders_detail.ID_request_menu !=''";
             }
         }
          ...
          if($orderStatus !== '' && $orderStatus !== 5){
                if($orderStatus == 0){
                    $str = $str." AND orders_detail.status = 0"; 
                    $str1 = " order by orders_detail.serve_at ASC";
                }elseif($orderStatus == 1){
                    $str = $str." AND orders_detail.status IN (1,2,3,4) AND orders_detail.serve_at >= NOW()";
                     $str1 = " order by orders_detail.serve_at ASC";
                }elseif($orderStatus == 2){
                    $str = $str." AND orders_detail.status IN (1,2,3,4) AND orders_detail.serve_at < NOW()";
                    $str1 = " order by orders_detail.serve_at DESC";
                }
             
          }
         
       $sql = "SELECT orders.id,setting.warn_serve_minute,orders_detail.serve_at,orders.created_at,orders_detail.status, user.name as client_name, user.email, client.phone as client_phone, orders.status,
        DATE_SUB(orders_detail.serve_at,  INTERVAL (SELECT setting.warn_serve_minute from setting WHERE setting.lang = restaurant.lang) minute) as sub_served_at,
        DATE_ADD(orders.created_at, INTERVAL (SELECT setting.warn_ordered_minute from setting WHERE setting.lang = restaurant.lang) minute) as sub_created_at,
        orders_detail.ID_menu_list as order_menu_list,orders_detail.ID_request_menu as order_request_menu,orders.update_by,
      ( 
      SELECT count(id) AS Quantity from orders_detail WHERE orders.id = orders_detail.ID_orders
      ) AS Quantity
      , (                                              
      SELECT SUM(price) as price from orders_detail WHERE orders.id = orders_detail.ID_orders 
      ) As prices
      , orders_detail.currency,restaurant.name as business_name,restaurant.phone As buss_phone, restaurant.SMS_phone 
      FROM orders 
      LEFT JOIN orders_detail ON orders.id = orders_detail.ID_orders 
      LEFT JOIN restaurant ON restaurant.id = orders.ID_restaurant 
      LEFT JOIN setting ON setting.lang = restaurant.lang
      LEFT JOIN client ON client.id = orders.ID_client 
      LEFT JOIN user ON user.id = client.ID_user
      LEFT JOIN district ON district.id = restaurant.ID_district 
      WHERE orders_detail.serve_at != '' AND setting.warn_serve_minute !='' ".$str."
      GROUP by orders.id,orders.ID_restaurant ".$str1;
        
       
         $result = DB::select( DB::raw($sql) );

         $sql1 = "SELECT orders.id,setting.warn_serve_minute,orders_detail.serve_at,orders.created_at,orders_detail.status, user.name as client_name, user.email, user.phone as client_phone, orders.status,menu_list.name as menu_name,orders_detail.x_number,
           DATE_SUB(orders_detail.serve_at,  INTERVAL (SELECT setting.warn_serve_minute from setting WHERE setting.lang = restaurant.lang) minute) as sub_served_at,orders_detail.price,orders_detail.currency,restaurant.name as business_name,restaurant.phone As buss_phone, restaurant.SMS_phone
         FROM orders
         LEFT JOIN orders_detail ON orders.id = orders_detail.ID_orders
         LEFT JOIN menu_list ON menu_list.id = orders_detail.ID_menu_list
         LEFT JOIN restaurant ON restaurant.id = orders.ID_restaurant
         LEFT JOIN setting ON setting.lang = restaurant.lang
         LEFT JOIN client ON client.id = orders.ID_client
         LEFT JOIN user ON user.id = client.ID_user
          LEFT JOIN district ON district.id = restaurant.ID_district
         WHERE orders_detail.serve_at != '' AND setting.warn_serve_minute !=''  ".$str." ".$str1;
    ...
    public static function getTurnovers($request, $isSum){
        ...
        $sql_before = "SELECT ID_restaurant,
        NAME,
        COUNT(*) orders,
    ...
    SELECT ID_restaurant,
     ...
     FROM (
            SELECT tt.*,
                ...
                bs.finance,
                (CASE WHEN tt.comm THEN tt.comm ELSE bs.finance END ) * tt.price / 100  AS commission                       
             FROM (
                SELECT
                    o.id,
                    o.ID_restaurant,
        ...
        $sql_middle = " GROUP BY o.ID_restaurant ) AS dd ON o.id_restaurant = dd.id_restaurant ";
        $sql_after = " )
                AS tt, bill_setting bs
                        WHERE bs.turnover - tt.turnover > 0
                        GROUP BY order_detail_id                        
                        ORDER BY ID_restaurant,id, ABS(distance) ) AS com
                        GROUP BY id ) AS rs
        ...
        if ( $cond_com_query != "" || $cond_pos_query != "" ||  $cond_status_query != "" ){
            $sql_con .= " WHERE 1 ".$cond_date_query.$cond_com_query.$cond_pos_query.$cond_status_query;

        }

        $sql = $sql_before.$sql_con.$sql_middle.$sql_con.$sql_after;
