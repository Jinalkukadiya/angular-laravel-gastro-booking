...
class PreregistrationController extends Controller
{
     ...
     public function confirmClient(Request $request)
    {
           ...
           $result = DB::table('orders')
                        ->where('id', $id)
                        ->update(['status' => 2, 'ID_user'=>1, 'update_by' => 'C','updated_at' =>date('Y-m-d H-i-s')]);
                    
            $result1 = DB::table('orders_detail')
                        ->where('ID_orders', $id)
                        ->update(['status' => 2]);
     ...
     public function confirmedBusiness(Request $request)
    {
       ...
       $id = $request->id; 
           ...
           $result = DB::table('orders')
                        ->where('id', $id)
                        ->update(['status' => 2, 'ID_user'=>1, 'update_by' => 'B','updated_at' =>date('Y-m-d H-i-s')]);
      ...
      public function cancelClient(Request $request)
    {
           ...
           $result = DB::table('orders')
                        ->where('id', $id)
                        ->update(['status' => 3, 'update_by' => 'C','updated_at' =>date('Y-m-d H-i-s')]);
      ...
      public function cancelBusiness(Request $request)
    {
           ...
           $result = DB::table('orders')
                        ->where('id', $id)
                        ->update(['status' => 3, 'update_by' => 'B','updated_at' =>date('Y-m-d H-i-s')]);
